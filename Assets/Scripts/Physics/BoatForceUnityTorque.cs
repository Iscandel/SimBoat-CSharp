...
class BoatForceUnityTorque : MonoBehaviour
{
    ...
    public float _maxPropellerRPM = 5000;
    private float _propellerEfficiency;
    public float _propellerDiameter;
    public Vector3 _thrustDirection;
    ...
    void FixedUpdate()
    {
        ...
        bool status = true;
        ForceTorque forceB = ComputeBuoyancy(ref status);
        if (!status)
            return;
        //allForces.AddRange(forcesB);
        ...
        _rigidbody.AddForce(forceB.force);
        _rigidbody.AddTorque(forceB.torque);

        //Debug.Log(this.name + " " + forceB.force + " " + forceB.torque);
        //Debug.Log("meth2" + " " + forceB.force.ToString("F10") + " " + forceB.torque.ToString("F10") + " " + Time.fixedTime);
    ...
    Force ComputeThrustForce()
    {
        ...
        Vector3 thrustDirection = _thrustDirection.normalized;

        Quaternion rotRudder = Quaternion.Euler(0, _rudderAngle, 0);
        thrustDirection = rotRudder * thrustDirection;

        Debug.DrawLine(worldThrustPosition, worldThrustPosition + transform.rotation * thrustDirection.normalized * 3, Color.magenta);

        float n2 = (_propellerRPM * MathTools.RPM_TO_RPS) * (_propellerRPM * MathTools.RPM_TO_RPS);
    ...
    //
    Force ComputePressureDragAtCenterOfPressure(Vector3 H, Vector3 M, Vector3 L, float hH, float hM, float hL, Vector3 normal)
    {
        ...
        if (H.y >= M.y)
        {
            ...
            if (depthCentroidUp > 0)
            {
                ...
                Vector3 center = GeometryTools.ComputeCentroid(H, M, P);
                Vector3 r = center - _rigidbody.worldCenterOfMass;
                Vector3 speedAtTriangle = _rigidbody.velocity + Vector3.Cross(_rigidbody.angularVelocity, r);
                float vi = speedAtTriangle.magnitude;
                float cosTheta = Vector3.Dot(speedAtTriangle.normalized, normal);
                if (cosTheta >= 0)
                {
                    fUp = -(_Cpd1 * vi / _vr + _Cpd2 * (float)Math.Pow(vi / _vr, 2)) * area * (float)Math.Pow(cosTheta, _fp);
                }
                else
                {
                    fUp = (_Csd1 * vi / _vr + _Csd2 * (float)Math.Pow(vi / _vr, 2)) * area * (float)Math.Pow(-cosTheta, _fs);
                }
            }
        }
        ...
        if (M.y > L.y)
        {
            ...
            if (depthCentroidDown > 0)
            {
                ...
                Vector3 center = GeometryTools.ComputeCentroid(L, M, P);
                Vector3 r = center - _rigidbody.worldCenterOfMass;
                Vector3 speedAtTriangle = _rigidbody.velocity + Vector3.Cross(_rigidbody.angularVelocity, r);
                float vi = speedAtTriangle.magnitude;
                float cosTheta = Vector3.Dot(speedAtTriangle.normalized, normal);
                if (cosTheta >= 0)
                {
                    fDown = -(_Cpd1 * vi / _vr + _Cpd2 * (float)Math.Pow(vi / _vr, 2)) * area * (float)Math.Pow(cosTheta, _fp);
                }
                else
                {
                    fDown = (_Csd1 * vi / _vr + _Csd2 * (float)Math.Pow(vi / _vr, 2)) * area * (float)Math.Pow(-cosTheta, _fs);
                }
            }
        }
    ...
    ForceTorque ComputeBuoyancy(ref bool status)
    {
        Vector3[] vertices = _mesh.vertices;
        ...
        if (!_waterProvider.SampleHeightAt(samplingList, ref verticesHeight))
        {
            ...
            status = false;
            ForceTorque res;
            res.force =  Vector3.zero;
            res.torque = Vector3.zero;
            return res;
        }
        ...
        ForceTorque resForce;
        resForce.force = Vector3.zero;
        resForce.torque = Vector3.zero;
    ...
    ForceTorque ComputeBuoyancyAtCenterOfPressure(Vector3 H, Vector3 M, Vector3 L, float hH, float hM, float hL, Vector3 normal)
    {
        ...
        force.torque = Vector3.Cross(appliPoint -_rigidbody.worldCenterOfMass, force.force);
...
//    void FixedUpdate()
//    {
//        //_waterProvider.SetSamplingTime(Time.fixedTime);Mai
...
//            currentTriNormal = transform.TransformDirection(currentTriNormal);
//            currentTriNormal.Normalize();
//            if (Vector3.Dot(geometricNormal, currentTriNormal) < 0)
//                Debug.LogWarning("Normal error..." + geometricNormal + " " + currentTriNormal);
...
//                    DepthTriangle waterTriangle1 = new DepthTriangle(il, M.vertex, L.vertex, heightIl, M.h, L.h, tmpNormal1);
//                    DepthTriangle waterTriangle2 = new DepthTriangle(im, M.vertex, il, heightIm, M.h, heightIl, tmpNormal2);

//                    if (!CompareNormals(tmpNormal1, geometricNormal))
//                    {
//                        waterTriangle1 = new DepthTriangle(il, L.vertex, M.vertex, heightIl, L.h, M.h, -tmpNormal1);
//                    }
//                    if (!CompareNormals(tmpNormal2, geometricNormal))
//                    {
//                        waterTriangle2 = new DepthTriangle(im, il, M.vertex, heightIm, heightIl, M.h, -tmpNormal2);
//                    }
...
//                    if (CompareNormals(tmpNormal, geometricNormal))
//                        _submergedTriangles.Add(new DepthTriangle(jh, jm, L.vertex, heightJh, heightJm, L.h, tmpNormal));
//                    else
//                        _submergedTriangles.Add(new DepthTriangle(jh, L.vertex, jm, heightJh, L.h, heightJm, -tmpNormal));
//                }
//            }
//            else if (H.h < 0 && M.h < 0 && L.h < 0)
//                _submergedTriangles.Add(new DepthTriangle(worldVertex0, worldVertex1, worldVertex2, height0, height1, height2, geometricNormal));
//        }

//        List<Force> forces = new List<Force>();
//        List<Vector3> forceVect = new List<Vector3>();
//        _submergedSurface = 0;
//        foreach (DepthTriangle triangle in _submergedTriangles)
//        {
//            (VertexAndDepth H, VertexAndDepth M, VertexAndDepth L) = triangle.GetSortedByVertex();
